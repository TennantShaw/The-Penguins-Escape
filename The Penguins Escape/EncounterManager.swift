...
class EncounterManager {
    ...
    // MARK: - Methods
    // We will call this addEncountersToScene function from the GameScene to append all of the encounter nodes to the world node from our GameScene:
    ...
    func placeNextEncounter(currentXPos: CGFloat) {
        ...
        // The game requires at least 3 encounters to function so exit this function if there are lesss than 3
        if encounterCount < 3 { return }
        // We need to pick an encounter that is not currently displayed on the screen:
        var nextEncounterIndex: Int?
        var trulyNew: Bool?
        // The current encounter and the directly previous encounter can potentially be on the screen at this time. Pick until we get a new encounter:
